...
class BlockMixin:
    ...
    def create_block_with_attributes(
        self,
        block_name: str,
        ...
        """Create a block definition with attribute definitions (ATTDEF).

        Uses COM API Blocks.Add() + AddAttribute() to create a block that
        contains attribute definitions. Optionally copies existing entities
        into the block.

        Args:
            block_name: Name for the new block
            attributes: List of attribute dicts, each with:
                - tag (str, required): Attribute tag name (e.g. "NO", "SIZE")
                - value (str, optional): Default value (default: "")
                - prompt (str, optional): Prompt text (default: tag name)
                - height (float, optional): Text height (default: 2.5)
                - mode (int, optional): Attribute mode (default: 0=Normal)
                    0=Normal, 1=Invisible, 2=Constant, 4=Verify, 8=Preset
                - insertion_point (tuple, optional): Relative position in block
                    (default: 0, -offset based on index)
            entity_handles: Optional list of entity handles to copy into block
            insertion_point: Base point for block definition (default: 0,0,0)
            description: Optional block description
        ...
        try:
            self._validate_connection()
            app = self._get_application("create_block_with_attributes")
            document = app.ActiveDocument
            ...
            if not attributes or not isinstance(attributes, list):
                raise InvalidParameterError(
                    ...
                    "attributes must be a non-empty list of dicts with 'tag' key"
                )
            ...
            # Copy entities into block if provided
            entities_added = 0
            failed_handles = []
            if entity_handles:
                entities = []
                ...
                if entities:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to copy entities to block: {e}")

            # Add attribute definitions via COM AddAttribute
            attr_tags_added = []
            for i, attr_def in enumerate(attributes):
                tag = attr_def.get("tag")
                if not tag:
                    logger.warning(f"Attribute {i} missing 'tag', skipped")
                    continue

                value = str(attr_def.get("value", ""))
                prompt = str(attr_def.get("prompt", tag))
                height = float(attr_def.get("height", 2.5))
                mode = int(attr_def.get("mode", 0))

                # Attribute insertion point (relative to block origin)
                attr_pt = attr_def.get("insertion_point")
                if attr_pt:
                    attr_coord = CADInterface.normalize_coordinate(attr_pt)
                else:
                    # Stack attributes vertically below origin
                    attr_coord = (0.0, -(i + 1) * (height * 1.5), 0.0)

                attr_pt_variant = self._to_variant_array(attr_coord)

                try:
                    block_def.AddAttribute(
                        height, mode, prompt, attr_pt_variant, tag, value
                    )
                    attr_tags_added.append(tag)
                    logger.debug(
                        f"Added ATTDEF: tag={tag}, value={value}, height={height}"
                    )
                except Exception as e:
                    logger.error(f"Failed to add attribute '{tag}': {e}")

            if not attr_tags_added:
                raise CADOperationError(
                    "create_block",
                    "No attribute definitions were added successfully",
                )

            logger.info(
                f"Created block '{block_name}' with {len(attr_tags_added)} ATTDEFs "
                f"and {entities_added} entities"
            )

            return {
                "success": True,
                "block_name": block_name,
                "attributes_added": attr_tags_added,
            ...
            logger.error(f"Failed to create block with attributes: {e}")
            raise CADOperationError("create_block", str(e))
    ...
    def insert_block(
        ...
        try:
            ...
            # Check if block exists (skip for file paths - COM API resolves them)
            is_file_path = "/" in block_name or "\\" in block_name or block_name.lower().endswith((".dxf", ".dwg"))
            block_exists = is_file_path  # Assume exists if file path
            if not is_file_path:
                try:
            ...
            # Insert the block
            block_ref = None
            used_sendcommand = False

            if is_file_path:
                # For file paths, try COM InsertBlock first, fall back to SendCommand
                try:
                    ...
                    logger.warning(
                        f"COM InsertBlock failed for file path '{block_name}': {e}. "
                        "Falling back to -INSERT SendCommand."
                    )
                    block_ref = self._insert_block_via_sendcommand(
                        document, block_name, point, scale_x, scale_y, rotation, layer, color,
                    )
                    used_sendcommand = True
            else:
                block_ref = document.ModelSpace.InsertBlock(
                    point_array,
                    block_name,
            ...
            if block_ref is None:
                raise CADOperationError(
                    "insert_block",
                    f"Failed to insert block '{block_name}': no entity created",
                )

            if not used_sendcommand:
                # Apply layer and color properties (SendCommand fallback handles this internally)
            ...
            logger.info(
                ...
                f"{' [via SendCommand]' if used_sendcommand else ''}"
            )
    ...
    def _insert_block_via_sendcommand(
        self,
        document: Any,
        file_path: str,
        point: "Point",
        ...
        """Insert a block from a DXF/DWG file using -INSERT SendCommand.

        Fallback for when COM InsertBlock doesn't support file paths (e.g. DXF files).

        Args:
            document: COM document object
        ...
        Returns:
            The inserted block reference COM object

        Raises:
            CADOperationError: If insertion fails
        """
        ...
        # Record entity count before insertion
        model_space = document.ModelSpace
        count_before = model_space.Count

        # Set target layer before insertion so the block lands on the right layer
        x, y = point[0], point[1]
        cmd = (
            f"FILEDIA 0\n"
            f"-LAYER\nS\n{layer}\n\n"
        ...
        logger.info(f"SendCommand -INSERT: {normalized_path} at ({x},{y})")
        document.SendCommand(cmd)

        # Poll for new entity (SendCommand is async)
        max_wait = 10.0
        poll_interval = 0.3
        elapsed = 0.0
        while elapsed < max_wait:
            ...
            current_count = model_space.Count
            if current_count > count_before:
                logger.info(
                    f"SendCommand -INSERT completed in {elapsed:.1f}s "
                    f"(entities: {count_before} -> {current_count})"
                )
                break
        else:
            raise CADOperationError(
                "insert_block",
                f"SendCommand -INSERT timed out after {max_wait}s. "
                f"Entity count unchanged at {count_before}.",
            )

        # Get the last inserted entity
        last_entity = model_space.Item(model_space.Count - 1)

        # Apply color (layer was set via -LAYER command before insertion)
        try:
            if isinstance(color, int):
                last_entity.Color = color
            elif isinstance(color, str) and color.lower() not in ("white", "bylayer"):
                color_map = {
                    "red": 1, "yellow": 2, "green": 3, "cyan": 4,
                    "blue": 5, "magenta": 6, "white": 7,
                }
                aci = color_map.get(color.lower())
                if aci is not None:
                    last_entity.Color = aci
        except Exception as e:
            logger.warning(f"Failed to set color on inserted block: {e}")
