...
function setupEventListeners() {
  document
    ...
    ?.addEventListener("click", handleManualRefresh);
  document
    .getElementById("export-all")
    ?.addEventListener("click", handleExport);
  document
    .getElementById("drawing-selector")
    ?.addEventListener("change", handleDrawingSwitch);

  document
    .getElementById("filter-entities")
    ?.addEventListener("input", renderEntities);
  document
    .getElementById("filter-entities-layer")
    ?.addEventListener("change", renderEntities);
  document
    .getElementById("filter-layers")
    ?.addEventListener("input", renderLayers);
  document
    .getElementById("filter-blocks")
    ?.addEventListener("input", renderBlocks);

  document
    .getElementById("expand-all-entities")
    ?.addEventListener("click", () =>
      toggleAllAccordions("entities-container", true),
    );
  document
    .getElementById("collapse-all-entities")
    ?.addEventListener("click", () =>
      toggleAllAccordions("entities-container", false),
    );
  document
    .getElementById("expand-all-blocks")
    ?.addEventListener("click", () =>
      toggleAllAccordions("blocks-container", true),
    );
  document
    .getElementById("collapse-all-blocks")
    ?.addEventListener("click", () =>
      toggleAllAccordions("blocks-container", false),
    );
}
...
async function handleExport() {
  ...
  try {
    ...
    if (result.success) {
      ...
      showToast(
        "Error al exportar: " + (result.detail || "Desconocido"),
        "error",
      );
    }
  } catch {
...
async function fetchEntitiesPage(type, page) {
  try {
    const res = await fetch(
      `/api/cad/entities?type=${encodeURIComponent(type)}&page=${page}&limit=${entitiesPerPage}`,
    );
    const data = await res.json();
    ...
    if (data.success) {
      ...
      const contentEl = document.querySelector(
        `.accordion-section[data-type="${type}"] .accordion-body`,
      );
      if (contentEl) {
        contentEl.innerHTML = "";
...
function updateUI() {
  ...
  // Panel count pills
  setText(
    "entities-panel-count",
    `${cadStatus.total_entities || 0} entidades totales`,
  );
...
function populateLayerFilter() {
  ...
  [...new Set(allLoadedEntities.map((e) => e.Layer).filter(Boolean))]
    .sort()
    .forEach((n) => {
...
function renderEntities() {
  ...
  const filterLayer =
    document.getElementById("filter-entities-layer")?.value || "";
...
function makeEntityTable(items) {
  ...
  const rows = items.map((e) => {
    ...
    if (
      String(effectiveColor).toLowerCase() === "bylayer" ||
      effectiveColor === 256
    ) {
      const entLayerNorm = String(e.Layer || "")
        .trim()
        .toLowerCase();
      const layerObj = layers.find(
        (l) =>
          String(l.Name || "")
            .trim()
            .toLowerCase() === entLayerNorm,
      );

      effectiveColor =
        layerObj &&
        layerObj.Color &&
        String(layerObj.Color).toLowerCase() !== "bylayer"
          ? layerObj.Color
          : "white";
...
function renderLayers() {
  ...
  if (!container) return;
  const ft = (
    document.getElementById("filter-layers")?.value || ""
  ).toLowerCase();
  const filtered = layers.filter((l) =>
    (l.Name || "").toLowerCase().includes(ft),
  );

  if (filtered.length === 0) {
    container.innerHTML =
      '<div class="empty-state">No hay datos de capas disponibles</div>';
    return;
  }
...
function renderBlocks() {
  ...
  if (!container) return;
  const ft = (
    document.getElementById("filter-blocks")?.value || ""
  ).toLowerCase();
  ...
  if (filtered.length === 0) {
    container.innerHTML =
      '<div class="empty-state">No hay bloques disponibles</div>';
    return;
  }
  ...
  filtered.forEach((b) => {
    ...
    container.appendChild(
      createAccordionSection(b.Name, `${b.Count || 0}×`, body),
    );
  });
}
