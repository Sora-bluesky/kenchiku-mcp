...
SHORTHAND FORMAT (one per line):
    ...
    insert with non-uniform scale (WxH):          → insert|Door|10,20|2.0x1.0|90|walls
...
def _insert(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scale_x = spec.get("scale_x", scale)
    scale_y = spec.get("scale_y", scale)
...
def _create_with_attrs(spec: Dict[str, Any]) -> Dict[str, Any]:
    """Create a block definition with attribute definitions (ATTDEF).

    Args:
        spec: Operation spec with keys: block_name (str), attributes (list or JSON str),
            entity_handles (list, optional), insertion_point (str, optional),
            description (str, optional).

    Returns:
        Dict with keys: success (bool), block_name (str), attributes_added (list),
            entities_added (int).
    """
    adapter = get_current_adapter()
    block_name = spec["block_name"]
    ...
    # Parse attributes (JSON string or list)
    ...
    # Normalize: accept both list-of-dicts and simple dict {"TAG": "value"}
    if isinstance(attrs_data, dict):
        attrs_data = [
            {"tag": k, "value": v} for k, v in attrs_data.items()
        ]

    if not isinstance(attrs_data, list):
        return {"success": False, "error": "attributes must be a list or dict"}

    # Parse entity handles if provided
    entity_handles = spec.get("entity_handles")
    if entity_handles is not None:
        if isinstance(entity_handles, str):
            entity_handles = [h.strip() for h in entity_handles.split(",")]
        if not isinstance(entity_handles, list):
            entity_handles = [entity_handles]
        entity_handles = [str(h) for h in entity_handles]

    result = adapter.create_block_with_attributes(
        block_name=block_name,
...
BLOCK_DISPATCH: Dict[str, Tuple[Callable, List[str]]] = {
    ...
    "create_with_attrs": (_create_with_attrs, ["block_name", "attributes"]),
    ...
    def manage_blocks(
        ...
        Args:
            operations: Operations in SHORTHAND format (one per line):
                ...
                insert|name|point|scale|rotation|layer|color  → insert|Door|10,20|1.5|90|walls|red
                create|name|handles|point|description         → create|MyBlock|A1,B2|0,0|Desc
                create_with_attrs|name|attrs_json|handles|pt|desc
                get_attrs|handle                              → get_attrs|A1B2C3
                set_attrs|handle|attributes_json              → set_attrs|A1B2C3|{"TAG": "value"}
                ...
                "attrs_json" for create_with_attrs = JSON: {"TAG":"default"} or [{"tag":"T","value":"v","height":2.5}]
                ...
                Examples:
                    ...
                    create_with_attrs|DOOR|{"NO":"","SIZE":"","TYPE":""}
                    create_with_attrs|DOOR|[{"tag":"NO","height":3},{"tag":"SIZE","height":3}]|A1,B2|0,0|Door block
                    get_attrs|A1B2C3
        ...
        for i, spec in enumerate(ops_data):
            ...
            try:
                ...
                if action_lower in ("create", "create_with_attrs", "insert"):
                    has_mutations = True
            except Exception as e:
