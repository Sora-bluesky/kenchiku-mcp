...
def _open(spec: Dict[str, Any]) -> Dict[str, Any]:
    """Open a drawing file.

    Args:
        spec: Operation spec with keys: filepath (str).

    Returns:
        Dict with keys: success (bool), detail (str).
    """
    filepath = spec["filepath"]
    success = get_current_adapter().open_drawing(filepath)
    if success:
        return {"success": True, "detail": f"Opened: {filepath}"}
    return {"success": False, "detail": f"Failed to open: {filepath}"}
    ...
    def manage_files(
        ...
        Args:
            operations: Operations in SHORTHAND format (one per line):
                ...
                open|filepath                 → open|C:/drawings/plan.dwg
                save|path_or_filename|format  → save|/path/to/file.dwg
                save|filename                 → save|backup.dwg
                new                           → new
                close|save_changes            → close|true
