...
BLOCK_OP_FIELDS: Dict[str, FieldList] = {
    ...
    "create_with_attrs": [
        ("block_name", True, None),
        ("attributes", True, None),  # JSON: {"TAG":"val"} or [{"tag":"T","value":"v"}]
        ("entity_handles", False, None),  # comma-separated handles
        ("insertion_point", False, "0,0"),
        ("description", False, ""),
    ],
    "get_attrs": [
...
def parse_block_op_shorthand(line: str) -> Dict[str, Any]:
    ...
    for i, (field_name, required, default) in enumerate(fields):
        ...
        if value_index < len(parts) and parts[value_index]:
            ...
            # Convert numeric fields
            if field_name == "scale":
                # Support non-uniform scale: "2.0x1.0" -> scale_x=2.0, scale_y=1.0
                if "x" in value.lower() and not value.lower().startswith("0x"):
                    parts_scale = value.lower().split("x")
                    if len(parts_scale) == 2:
                        spec["scale_x"] = _try_number(parts_scale[0])
                        spec["scale_y"] = _try_number(parts_scale[1])
                        spec["scale"] = spec["scale_x"]  # fallback
                    else:
                        spec[field_name] = _try_number(value)
                else:
                    spec[field_name] = _try_number(value)
